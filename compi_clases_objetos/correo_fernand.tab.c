...
{
  "$end", "error", "$undefined", "CADENA", "$accept", "lista", YY_NULLPTR
};
#endif
...
#line 1199 "correo_fernand.tab.c"
...
//analizador lexico hecho a mano
	...
	while(1){
	    ...
	    if(isalpha(c) && (c=='a' || c == 'b')){//verifica si es un nro. entero
			int i=0;
			do{
				lexema[i++]=c;
				c=getchar();
			}while(isalpha(c) && (c=='a' || c == 'b'));
		ungetc(c,stdin);//devuelve el caracter a la entrada estandar
		lexema[i]=0;
		return CADENA; // devuelve el token
		}
			return c;
